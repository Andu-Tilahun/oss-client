...
export class SmsTemplatesPageComponent implements OnInit, OnDestroy {
  ...
  readonly purposeOptions: PurposeOption[] = PURPOSE_OPTIONS['SMS'];
  ...
  load(): void {
    this.loading = true;
    this.service.getTemplates('SMS').subscribe({
      next: (list) => {
        this.templates = list;
        this.loading = false;
  ...
  openCreate(): void {
    this.editingId = null;
    this.form.reset({ active: true, defaultTemplate: false, name: '', purpose: null, body: '' });
    this.showModal = true;
  }
  ...
  openEdit(t: MessageTemplate | null): void {
    if (!t) return;
    this.editingId = t.id;
    this.form.patchValue({ name: t.name, purpose: t.purpose || null, body: t.body, active: t.active, defaultTemplate: t.defaultTemplate });
    this.showModal = true;
  }
  ...
  save(): void {
    ...
    this.saving = true;
    const request: MessageTemplateRequest = { ...this.form.value, type: 'SMS' };
