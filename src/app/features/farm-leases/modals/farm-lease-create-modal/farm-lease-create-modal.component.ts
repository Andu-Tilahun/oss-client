...
import {LeaseCreateRequest, LeaseGenerateTermsRequest} from '../../models/farm-lease.model';
...
export class FarmLeaseCreateModalComponent {
  ...
  onPaymentSubmit(request: LeaseGenerateTermsRequest): void {
    if (!this.leaseId) return;
    this.isSaving = true;
    this.farmLeaseService.generateTerms(this.leaseId, request).subscribe({
      next: (lease) => {
        this.currentStep = 3;
        this.isSaving = false;
      },
